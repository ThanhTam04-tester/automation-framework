...
def wait_for_preloader(driver):
    ...
    time.sleep(2) # Tăng thời gian chờ load
...
class TestPalatinUI:
    ...
    def test_05_06_booking_flow(self, driver, config):
        ...
        # 1. Kiểm tra xem có phòng trống không
        room_areas = driver.find_elements(By.CLASS_NAME, "single-rooms-area")
        has_empty = any("Trống" in area.text for area in room_areas)
        
        # Nếu HẾT PHÒNG TRỐNG -> Chạy API ngầm ép Reset phòng số 1 để có cái test
        if not has_empty:
            client = requests.Session()
            client.post(config["base_url"] + "/login", data={"email": "admin@gmail.com", "password": "123"})
            client.post(config["base_url"] + "/admin/rooms/reset/1")
            driver.refresh()
            wait_for_preloader(driver)
            time.sleep(3)
        
        # 2. Lấy link các phòng đang "Trống"
        empty_room_links = []
        room_areas = driver.find_elements(By.CLASS_NAME, "single-rooms-area")
        ...
        # 3. Vô phòng trống đầu tiên để Test cảnh báo điền thiếu thông tin
        ...
        driver.execute_script("document.getElementById('detailCustPhone').value = '';") # Cố tình bỏ trống
        ...
        # 4. Lặp qua tất cả các phòng trống và Đặt thành công
    ...
    def test_08_admin_add_new_room(self, driver, config):
        ...
        # MỞ TAB PHÒNG BẰNG JS THUẦN TÚY (Đã sửa lỗi jQuery $)
